function validateSeries(data, period) {
  if (
    !Array.isArray(data) ||
    ...
    throw new RangeError(
      "Finite series and positive integer period required / série finita e período inteiro positivo",
    );
  }
}
...
export function bollingerBands(closes, period = 20, stdDevMultiplier = 2) {
  if (!Number.isFinite(stdDevMultiplier) || stdDevMultiplier < 0)
    throw new RangeError(
      "Invalid deviation multiplier / multiplicador inválido",
    );
...
export function vwap(candles) {
  ...
  for (const candle of candles) {
    if (
      ![candle.high, candle.low, candle.close, candle.volume].every(
        Number.isFinite,
      ) ||
      candle.volume < 0 ||
      ...
      throw new RangeError("Invalid candle / candle inválido");
    const typicalPrice = (candle.high + candle.low + candle.close) / 3;
    cumulativeVolumePrice += typicalPrice * candle.volume;
    cumulativeVolume += candle.volume;
    result.push(
      cumulativeVolume > 0 ? cumulativeVolumePrice / cumulativeVolume : null,
    );
  }
